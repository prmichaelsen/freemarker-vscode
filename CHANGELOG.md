#### [v0.1.8] fix: Remove dead command declarations — `addFile` + `checkForUpdates`
- Bug: `package.json` declared two commands in `contributes.commands` — `freemarker-vscode.addFile` ("FreeMarker: Add file") and `freemarker-vscode.checkForUpdates` ("FreeMarker: Check for updates") — that had no `registerCommand(...)` site anywhere under `client/src/`. Both surfaced in the VS Code Command Palette and failed with `command 'freemarker-vscode.addFile' not found` (or `…checkForUpdates`) when invoked. The two declarations have been in `package.json` since v0.0.4 (March 2025); the corresponding implementations were never authored. Logged as a side find in the v0.1.7 wake report.
- Fix: remove both declarations from `contributes.commands` in `package.json`. The Command Palette no longer surfaces them; users can no longer invoke them and see the "not found" error. The remaining contributed command (`freemarker-vscode.debugParser`, "FreeMarker: Debug parser") is unchanged and continues to be registered by `registerDebugParserCommand.ts`.
- Doc: README.md "Commands" section trimmed from three entries to one (`Debug parser`). The v0.1.6 README had listed all three because they were declared in `package.json`; the README and `package.json` are now consistent and accurate about what actually works.
- Semantics note: VS Code Marketplace handles extension auto-updates via its own UI surfaces. The original `checkForUpdates` declaration appears to predate Marketplace publish (the extension still has not been published to the Marketplace as of v0.1.8); the duplication is not desirable in either case. If self-update is wanted later for the `.vsix`-from-GitHub-releases install path, the right surface is a one-time activation-time check, not a manually-invoked command. `addFile` had no observable semantic over VS Code's built-in "New File" command. Both are removable without loss.
- Internal registered commands not declared in `package.json`: `freemarker-vscode.telemetry.acceptCompletion` (registered by `registerAcceptCompletionCommand.ts`) is intentionally undeclared — it is an internal hook invoked from `CompletionItem.command`, not a user-facing palette entry. Surfacing it in the palette would let users invoke it manually with no payload, which is wrong; leaving it undeclared is the correct shape.
- Tests: no test-surface change — neither command had a code path under test. `tsc -b` clean; `npm --prefix server test` green (30 / 30).
